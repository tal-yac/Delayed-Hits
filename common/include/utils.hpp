...
namespace utils {
static char constexpr trace_delimiter = ';';
...
class Packet {
  ...
  public:
	...
	inline bool checkNotFinalized() {
		if (is_finalized_) {
			throw std::runtime_error("Cannot modify a finalized packet.");
		}
...
class FlowData {
  public:
	...
	size_t getIdxRange() const {
		return indices_.empty() ? 0 : (indices_.back() - indices_.front());
	}
	// Mutators
 ...
 */
std::vector<std::string> parseTrace(const std::string& trace_fp) {
	...
	while (std::getline(trace_ifs, line)) {
		...
		if (!line.empty()) {
			...
			std::getline(linestream, timestamp, trace_delimiter);
			std::getline(linestream, flow_id, trace_delimiter);
		}
...
class TraceAnalyzer {
  ...
  public:
	TraceAnalyzer(const std::string& trace_fp) : trace_fp_(trace_fp) {
		analyzeFlowArrivals();
	}
	...
	const std::unordered_map<std::string, FlowData>& getFlowIdsToDataMap()
		const {
		return flow_ids_to_data_map_;
	}
};
